...
import { getCirculatingSupply } from "./ProtocolMetric";
...
export function updateSimpleStaking(timestamp: string): void {
  ...
  const rebaseRateApyAndIndex = getRebaseRateApyAndIndex(
    simpleStaking.stakedSupply
  );
  ...
  simpleStaking.stakedPercentage = simpleStaking.stakedSupply
    .div(getCirculatingSupply())
    .times(BigDecimal.fromString("100"));
  simpleStaking.timestamp = timestamp;
  simpleStaking.save();
}

function getStakedSupply(): BigDecimal {
  const sExodiaContract = SEXODERC20.bind(
...
function getRebaseRateApyAndIndex(stakedSupply: BigDecimal): BigDecimal[] {
  const exodStakingContract = ExodStaking.bind(
  ...
  if (stakedSupply.gt(BigDecimal.zero())) {
    const rebaseRate = nextDistributedExod
      .div(stakedSupply)
      .times(BigDecimal.fromString("100"));
    ...
    const apy = BigDecimal.fromString(
      (
        (Math.pow(1 + parseFloat(rebaseRate.toString()) / 100, rebasesPerYear) -
          1) *
        100
      ).toString()
    );
